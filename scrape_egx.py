...
def human_delay(min_s=4.5, max_s=7.0):
...
def main():
    ...
    with sync_playwright() as p:
        ...
        # --- PART 1: SCRAPE ALL INDICES VIA NATURAL CLICK INTERACTIONS ---
        print("Navigating to Indices Workspace...")
        try:
            page.goto("https://www.egx.com.eg/en/Indices.aspx", wait_until="networkidle", timeout=50000)
            human_scroll(page)
            page.wait_for_timeout(3000)

            # Map the actual DOM link text element wrappers rather than raw string __doPostBacks
            click_targets = {
            ...
            for tracking_name, selector in click_targets.items():
                ...
                if page.locator(selector).count() > 0:
                    ...
                    page.wait_for_timeout(random.uniform(4000, 5500))
                    ...
                    if metrics.get("value") is not None:
                        ...
                        print(f"[-] Metric text extraction missed on panel {tracking_name}")
                else:
                    print(f"[-] Selector {selector} not mounted in page layout context.")
        except Exception as loop_error:
            print(f"[-] Stop on indices interface: {loop_error}")
        ...
        # --- PART 8: LIVE STATUS & INDEX CHART GRAPH DATA ---
        ...
        for index_name, target_url in constituent_targets.items():
            ...
            try:
                ...
                # Check for table visibility, wait up to 10 seconds
                page.wait_for_selector("#ctl00_C_CIC_GridView1", timeout=10000)
                page.wait_for_timeout(3000) 
