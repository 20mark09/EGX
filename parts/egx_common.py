...
def fetch_investor_json(page, base_url, params, referer, retries=2, retry_delay=3):
    """Fetches one investor-activity WebService.asmx endpoint.

    IMPORTANT: this POSTs a JSON body rather than GET-with-querystring.
    Classic ASP.NET AJAX-enabled [WebMethod]s only accept GET if they're
    explicitly decorated with [ScriptMethod(UseHttpGet = true)]; by
    default they require POST with a JSON body
    (Content-Type: application/json). A GET to a POST-only WebMethod can
    get rejected before any HTTP response is even framed - which shows
    up as a Playwright APIRequestContext exception, or, if issued via a
    real page.evaluate(fetch(...)), as a bare "TypeError: Failed to
    fetch" with no status code at all. That symptom (not a 403/406, but
    *no response whatsoever*, identically on the very first attempt,
    from both Playwright's HTTP client and real Chromium fetch()) is
    what GET-based calls to these endpoints produced - which points at
    the request shape being wrong rather than at bot-fingerprinting.

    POST-style ASP.NET AJAX WebMethod responses are also typically
    wrapped in a {"d": ...} envelope (unlike the GET convention's raw
    JSON), so we unwrap that here.

    These endpoints have also independently been observed occasionally
    ...
    a request-shape or scraping problem. We retry a couple of times
    before giving up, and log the actual bad response so a future empty
    result is diagnosable straight from the Action log.
    """
    for attempt in range(retries + 1):
        try:
            result = page.evaluate(
                ...
                {"url": base_url, "body": json.dumps(params)},
            )
            ...
            if stripped.startswith("{") or stripped.startswith("["):
                try:
                    ...
                    # Standard ASP.NET AJAX WebMethod envelope: the real
                    # payload lives under "d", either as a native
                    # array/object or as a JSON-encoded string.
            ...
            print(f"[-] Unusable response from {base_url} "
                  f"(status {result.get('status') if result else '?'}, "
                  f"attempt {attempt + 1}/{retries + 1}): {stripped[:150]!r}")
        except Exception as e:
            print(f"[-] Failed to fetch {base_url} (attempt {attempt + 1}/{retries + 1}): {e}")
