...
def scrape_prices(page):
    ...
    try:
        ...
        if link_count > 0:
            ...
            def handle_postback_request_failed(request):
                if request.method == "POST" and "prices.aspx" in request.url:
                    print(f"[debug] postback request FAILED (no response received): "
                          f"url={request.url} failure={request.failure}")

            page.on("response", handle_postback_response)
            page.on("requestfailed", handle_postback_request_failed)
            ...
            page.remove_listener("requestfailed", handle_postback_request_failed)
